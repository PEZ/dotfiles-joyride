...
(comment
  (def panel (-> (vscode/commands.executeCommand "workbench.view.extension.joyride-activitybar")
                 (.then (fn [x]
                          (.appendLine (joyride/output-channel x))))
                 (.catch (fn [e]
                           (.appendLine (joyride/output-channel) e)))))
  (.-name panel)
  (js-keys panel)
  :rcf)
